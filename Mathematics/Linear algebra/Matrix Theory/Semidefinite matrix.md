...
A note on the structure of $A$, if $A$ is positive semidefinite, then $a_{ii}=0$ it is necessary that $A_{ij}=0$ and $a_{ji}=0$ for all $j$; that is, the row and column of the zero diagonal entry are all zeros.
