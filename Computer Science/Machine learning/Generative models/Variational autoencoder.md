...
During training, under no additional constraints, we attempt to minimize the reconstruction error between inputs and the generated output: $$\arg\min_{\theta,\phi}\frac{1}{2}\sum_{i=1}^N \|g_\phi(f_\theta(x))-x_i\|^2$$
